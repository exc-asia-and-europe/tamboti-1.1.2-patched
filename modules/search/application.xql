...
declare function biblio:last-collection-queried($node as node(), $params as element(parameters)?, $model as item()*) {
        ...
        let $search-collection := 
            ...
            then replace(replace(xmldb:decode-uri($search-collection), $config:mods-commons, $config:data-collection-name), $config:users-collection, $config:data-collection-name)
